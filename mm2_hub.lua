task.spawn(function()
    ...
    if not PlayerGui then
        warn("[UltimateMM2 Hub]: Не удалось найти PlayerGui!")
        return
    end

    -- Удаляем старое меню при перезапуске
    local oldGui = PlayerGui:FindFirstChild("UltimateMM2_Menu")
    if oldGui then
        oldGui:Destroy()
    end
    ...
    -- Монтируем в PlayerGui (или CoreGui, если поддерживается)
    pcall(function()
        if gethui then
    ...
    -- Кнопка открытия при сворачивании
    ...
    -- Универсальный обработчик нажатия для ПК и мобильных устройств
                ...
                if myRole == "Murderer" or myRole == "Sheriff" then
                    ...
                    for _, p in ipairs(Players:GetPlayers()) do
                        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                            ...
                            local shouldTarget = (myRole == "Murderer" and targetRole == "Sheriff") or (myRole == "Sheriff" and targetRole == "Murderer")
            ...
            if autoFarmCoinsEnabled then
                pcall(function()
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        ...
                        local targets = coinContainer and coinContainer:GetChildren() or Workspace:GetDescendants()
                        
                        for _, coin in ipairs(targets) do
